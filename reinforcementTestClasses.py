...
import testClasses
import sys, os
import layout, graphicsDisplay, pacman, gridworld
import time
...
class ValueIterationTest(testClasses.TestCase):
    ...
    def runAgent(self, moduleDict, numIterations):
        ...
        agent = valueIterationAgents.ValueIterationAgent(self.grid, discount=self.discount, iterations=numIterations)
...
class PolicyIterationTest(ValueIterationTest):
    def __init__(self, question, testDict):
    ...
    def runAgent(self, moduleDict, numIterations):
        ...
        agent = policyIterationAgent.PolicyIterationAgent(self.grid, discount=self.discount, iterations=numIterations)
        states = self.grid.getStates()
        actions = list(reduce(lambda a, b: set(a).union(b), [self.grid.getPossibleActions(state) for state in states]))
        values = {}
        qValues = {}
        ...
        for state in states:
            ...
            if state not in agent.policy:
                raise ValueError(f"No policy known for state '{state}', make sure you populate the agent's policy in runPolicyIteration.")
        ...
        valuesPretty = self.prettyValues(values)
        policyPretty = self.prettyPolicy(policy)
        iterations_to_converge = agent.iterations_to_converge
        qValuesPretty = {}
        for action in actions:
            qValuesPretty[action] = self.prettyValues(qValues[action])
        return (valuesPretty, qValuesPretty, actions, policyPretty,iterations_to_converge)
    ...
    def executeNIterations(self, grades, moduleDict, solutionDict, n, checkPolicy):
        testPass = True
        valuesPretty, qValuesPretty, actions, policyPretty, iterations_to_converge = self.runAgent(moduleDict, n)
        stdOutString = ''
        fileOutString = ''
        valuesKey = "values_k_%d" % n
        ...
        for action in actions:
            ...
            if self.comparePrettyValues(qValues, solutionDict[qValuesKey]):
                ...
                stdOutString += outString
                fileOutString += outString
        convergence_key = "iterations_to_converge"
        target_convergence= min(n,int(solutionDict[convergence_key]))
        if not iterations_to_converge == target_convergence:
            testPass=False
            outString = f"The number of completed iterations is NOT correct for i={n}. This might be because one of updateValues or updatePolicy does not correctly check for convergence.\n"
            outString += "   Student solution: %s\n" %  str(iterations_to_converge)
            outString += "   Correct solution: %s\n" %  str(target_convergence)
            stdOutString += outString
            fileOutString += outString
        if checkPolicy:
        ...
        for x in range(width):
            for y in range(height):
                if self.policy[x][y] in actionMap and policy[(x,y)] != actionMap[self.policy[x][y]]:
                    differPoint = (x,y)
                    pValueIterationTestolicyPassed = False
